...
class UserCalendar
{
	...
	function UserCalendar($user_id, $month, $year)
	{
	...
	function getEvents()
	{
		...
		$fields = array($this->user_id, $this->month, $this->year);
		$query = "SELECT DISTINCT on (name, startdate)
					event_id,
					name,
					...
					user_id
					FROM events
					WHERE active = 'true' AND user_id = $1 AND to_char(startdate, 'mm') = $2 AND to_char(startdate, 'yyyy') = $3";
		$db->query($query, $fields);
		
		$events = array();
		for ($i = 0; $row = $db->fetch(); $i++)
		{
//			$truncateDescription = $row['event_img'] == "" ? 900 : 450;
			$truncateDescription = 450;
		...
		for($list_day = 1; $list_day <= $days_in_month; $list_day++):
			...
			else
			{
				...
				if (count($dayEvents) > 0)
				{
					...
					$url = "/day.php?user_id=". $this->user_id ."&month=". $this->month ."&day=". $urlDay ."&year=". $this->year;
				...
				for ($i = 0; $i < count($dayEvents); $i++)
				{
					...
					$title.= "<h3>". $dayEvents[$i]['name'] ."</h3><h3>". $dayEvents[$i]['time'] ." at ". $dayEvents[$i]['location'] ."</h3><p>". $dayEvents[$i]['description'] ."</p></div>";
					$calendar.= "<li><a href=\"/event.php?id=". $dayEvents[$i]['event_id'] ."&amp;ret=user\" class=\"event\" title=\"$title\">". $eventName ."</a></li>";
				}
